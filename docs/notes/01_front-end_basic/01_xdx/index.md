...
> PS：程序员也会自嗨，称自己为码农、码畜、码神、IT 民工、开发者等等。
...
> PS：现在前端已经很少去兼容其它浏览器了，通常而言，只需要兼容 Chrome 即可；在后期，可以通过 [PostCSS](https://postcss.org/) 等来帮助我们生成兼容的代码。
...
> PS：之所以有 `.htm`，是因为历史遗留问题，Win95 或 Win98 系统的文件扩展名不支持超过 3 个字符，所以只能使用 `.htm`。
...
* 记事本是否可以开发网页？
* 可以，但是有很多缺点：① 创建和管理文件不方便 ② 没有颜色标识，没有智能提示，无法调试程序。
...
> PS：目前，前端一般使用 `Visual Studio Code`，主要是因为开源免费，并且插件丰富。
...
> PS：我们会发现 HTML 元素非常之多，怎么记住？其实，不用太刻意去记忆，毕竟实际开发中，使用到的就那么几个，用多了，也就记住了。
...
> PS：一个元素中可以有一个属性，也可以没有属性，也可以包含多个属性。
